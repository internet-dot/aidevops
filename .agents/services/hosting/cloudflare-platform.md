...
> **Not for API operations**: To manage, configure, or update Cloudflare resources (DNS records, zone settings, deployments) use the Cloudflare Code Mode MCP — see `.agents/tools/api/cloudflare-mcp.md`.
...
- **Not for**: Managing/configuring CF resources → use `.agents/tools/api/cloudflare-mcp.md` (Code Mode MCP)
...
> **API & configuration details**: Use the Cloudflare Code Mode MCP (`.agents/tools/api/cloudflare-mcp.md`) for live OpenAPI spec queries — `api.md` and `configuration.md` files have been removed as they are superseded by Code Mode's real-time spec access.
